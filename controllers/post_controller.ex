...
{:ok, _} ->
conn
|> put_flash(:info, "Post was created
successfully")
...
render(conn, "edit.html", post: post, changeset:
changeset)
else
conn
...
{:ok, _} ->
conn
|> put_flash(:info, "Post was updated
successfully")
|> redirect(to: user_post_path(conn, :show,
current_user.id,
post.id))
{:error, changeset} ->
render(conn, "edit.html", post: post, changeset:
